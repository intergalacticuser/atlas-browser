#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════
# Angle Browser - One-Line Installer for macOS
#
# Usage:
#   curl -sL https://raw.githubusercontent.com/intergalacticuser/angle-browser/main/install.sh | bash
#
# Or with wget:
#   wget -qO- https://raw.githubusercontent.com/intergalacticuser/angle-browser/main/install.sh | bash
# ═══════════════════════════════════════════════════════════════
set -euo pipefail

REPO="intergalacticuser/angle-browser"
APP_NAME="Angle Browser"
DMG_NAME="Angle-Browser.dmg"
...
echo ""
echo -e "${CYAN}${BOLD}  ╔══════════════════════════════════════╗${NC}"
echo -e "${CYAN}${BOLD}  ║       Angle Browser Installer        ║${NC}"
...
if [[ "$(uname)" != "Darwin" ]]; then
  echo -e "${RED}Error: Angle Browser is currently macOS only.${NC}"
...
echo -e "${DIM}> Downloading Angle Browser...${NC}"
...
echo ""
echo -e "${GREEN}${BOLD}  ✓ Angle Browser installed successfully!${NC}"
echo ""
...
# Offer to launch
read -p "  Launch Angle Browser now? [Y/n] " -n 1 -r
echo ""
if [[ $REPLY =~ ^[Yy]?$ ]]; then
  open -a "$APP_NAME"
